...
class FullyConnectedLayer(object):
    def __init__(self, num_input, num_output, lambda_):  # 全连接层初始化
    ...
    def backward(self, top_diff):  # 反向传播的计算
        ...
        self.d_weight = np.dot(self.input.T, top_diff) + self.lambda_*self.weight
        self.d_bias = np.sum(top_diff, axis=0) + self.lambda_*self.bias
